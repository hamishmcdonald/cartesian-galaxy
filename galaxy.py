...
read_file_directory = 'E:\GaiaSource'

#local file location which xyz files will be written to
write_file_directory = 'E:/xyz'
...
def main():
    ...
    for gaia_file in os.listdir(read_file_directory):
        if gaia_file.endswith('.csv'):
            try:
                with open(read_file_directory + '/' + gaia_file, newline='') as current_csv:
                    ...
                    current_reader = csv.DictReader(current_csv)
                    
                    current_write_file = open(write_file_directory + '/gaia-source-' + gaia_file[11:24] + '.xyz', 'w', newline='')

                    #iterate through each star in the current GaiaSource file
                    for i, row in enumerate(current_reader, start = 1):
                        try:
                ...
                #close the current xyz file being written to
...
def calculate_cartesian(row):
    ...
    return str(x_value), str(y_value), str(z_value)
